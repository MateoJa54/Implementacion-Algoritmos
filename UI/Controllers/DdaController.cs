...
namespace ImplementaciónAlgoritmos.UI.Controllers
{
    public class DdaController
    {
        ...
        public DdaController(PictureBox canvas, DataGridView grid)
        {
            _canvas = canvas;
            _grid = grid;
            _algorithm = new DdaLineAlgorithm();
        ...
        private void HandlePixel(Core.Models.Pixel p)
        {
            // center offset
            ...
            if (_canvas.InvokeRequired)
                _canvas.Invoke(new Action(() =>
                    _canvas.CreateGraphics().FillEllipse(Brushes.Red, drawX, drawY, 2, 2)
                ));
            else
                _canvas.CreateGraphics().FillEllipse(Brushes.Red, drawX, drawY, 2, 2);
